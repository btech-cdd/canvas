...
async function generateDetailedContent(
  ...
  containerEl.html(`
    ...
    <div v-if="view == 'unaligned'">
      <div v-for="(page, p) in pageReviewsData.filter(page => (page?.objectives ?? []).length == 0 && page.include)" :key="p"><a :href="'https://btech.instructure.com/courses/' + page.course_id + '/pages/' + page.page_id">{{page.name}}</a></div>
      <div v-for="(assignment, a) in assignmentReviewsData.filter(assignment => (assignment?.objectives ?? []).length == 0 && assignment.include)" :key="a"><a :href="'https://btech.instructure.com/courses/' + assignment.course_id + '/assignments/' + assignment.assignment_id">{{assignment.name}}</a></div>
    </div>
  `);
  if (courseReviewData) {
    let APP = new Vue({
