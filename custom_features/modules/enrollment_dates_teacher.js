(async function() {
  ...
  $("#btech-enrollment-view-all-dates").click(async () => {
    ...
    for (let d in dates) {
      let date = dates[d];
      console.log(date);
      modalContent.append(`<div><span style="width: 2.5rem; display: inline-block;">${date.is_extension ? '<b>EXT</b>' : ''}</span><span style="width: 6rem; display: inline-block;">${dateToString(date.end_date)}</span>Created By: ${date.creator_name} @ ${dateToString(date.created)}</div>`)
    }
